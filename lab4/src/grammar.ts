...
export class Grammar implements IGrammar {
    ...
    private replaceNontermLeftRecProductionsForNonterm(firstNonterm: Nonterm, secondNonterm: Nonterm) {
        console.log(firstNonterm, secondNonterm)
    ...
    public operatorPrecedenceGrammar() {

        for (let j = 0; j < this.productions.length; j++) {
            console.log(j)

            const value = Object.values(this.productions[j])[0]
            const key = Object.keys(this.productions[j])[0]
            for (let i = 0; i < value.length - 1; i++) {
                if (value.length === 1) {
                    break;
                }
                if (this.nonterm.includes(value[i]) && this.nonterm.includes(value[i + 1])) {
                    if (value[i] !== key) {
                        // const secondNontermProductions = this.getNontermProductions(value[i])
                        // secondNontermProductions.forEach(secondProduction => {
                            // this.productions.push({})
                        // })
                        // this.productions.splice(this.productions.indexOf(production), 1)

                        this.replaceNontermLeftRecProductionsForNonterm(value[i + 1], value[i])
                    } else {
                        this.replaceNontermLeftRecProductionsForNonterm(value[i], value[i + 1])
                    }
                    console.table(value)
                    j = 0;
                    break;
